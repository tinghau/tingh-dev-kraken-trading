...
public class BookDataHandler {
    ...
    public void handleBookData(String jsonData) {
        try {
            BookData bookData = JsonUtils.fromJson(jsonData, BookData.class);
        ...
        try {
            ...
            if (!fileExists) {
                String headers = "timestamp,symbol,update_type," +
                        buildPriceLevelHeaders("bid") + "," +
                        buildPriceLevelHeaders("ask") + "\n";
                Files.writeString(filePath, headers, StandardOpenOption.CREATE, StandardOpenOption.WRITE);
            }

            // Get top N bids (highest to lowest)
            List<BookData.PriceLevel> topBids = getTopN(state.bids.values(), true);

            // Get top N asks (lowest to highest)
            List<BookData.PriceLevel> topAsks = getTopN(state.asks.values(), false);
            ...
            // Add bid data
            appendPriceLevels(dataRow, topBids);

            // Add ask data
            appendPriceLevels(dataRow, topAsks);
    ...
    private String buildPriceLevelHeaders(String side) {
        StringBuilder headers = new StringBuilder();
        for (int i = 0; i < BookDataHandler.MAX_DEPTH; i++) {
    ...
    private List<BookData.PriceLevel> getTopN(Collection<BookData.PriceLevel> levels, boolean descending) {
        Comparator<BookData.PriceLevel> comparator = Comparator.comparing(
                level -> new BigDecimal(level.getPrice())
        );
        ...
        return levels.stream()
                .sorted(comparator)
                .limit(BookDataHandler.MAX_DEPTH)
                .toList();
    }

    private void appendPriceLevels(StringBuilder sb, List<BookData.PriceLevel> levels) {
        for (int i = 0; i < BookDataHandler.MAX_DEPTH; i++) {
            sb.append(",");
