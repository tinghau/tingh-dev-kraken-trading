...
public class OrderDataHandler {
        ...
        try {
            ...
            if (!fileExists) {
                String headers = "timestamp,symbol," +
                        ...
                        buildOrderHeaders("bid") + "," +
                        buildOrderHeaders("ask") + "\n";
            ...
            // Add bid orders
            appendOrders(dataRow, sortedBids);

            // Add ask orders
            appendOrders(dataRow, sortedAsks);
    ...
    private String buildOrderHeaders(String side) {
        StringBuilder headers = new StringBuilder();
        for (int i = 0; i < OrderDataHandler.MAX_ORDERS_TO_WRITE; i++) {
    ...
    private List<OrderData.OrderUpdate> sortOrders(Collection<OrderData.OrderUpdate> orders, boolean descending) {
        Comparator<OrderData.OrderUpdate> comparator = Comparator.comparing(
                order -> new BigDecimal(order.getLimitPrice())
        );
    ...
    private void appendOrders(StringBuilder sb, List<OrderData.OrderUpdate> orders) {
        for (int i = 0; i < OrderDataHandler.MAX_ORDERS_TO_WRITE; i++) {
            sb.append(",");
